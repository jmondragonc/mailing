...
async function generateImages(dealer, cliente, asesor, telefono_asesor, id) {
  ...
  const sFile = "base.png", // source image
    sSave = `${id}_header_${fileName.toLocaleLowerCase()}.png`; // "save as"

  console.log(id + "_header_" + fileName.toLocaleLowerCase());
  ...
  await loadImage(sFile).then((img) => {
    ...
    async function generate() {
      try {
        const result = await drawText(cliente, "32px", 205, 0);
        console.log(`Got the result1: ${result}`);
        // if (result) {
        //   try {
        //     const result2 = await drawText(asesor, "20px", 230, 0);
        //     if (result2) {
        //       console.log(`Got the result2: ${result2}`);
        //       try {
        //         const result3 = await drawText(dealer, "19px", 265, 0);
        //         if (result3) {
